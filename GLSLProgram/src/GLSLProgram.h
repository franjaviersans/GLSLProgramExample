...
{
public:
    enum SHADERTYPE {VERTEX = 0, TESSELLATION_CONT = 1, TESSELLATION_EVAL  = 2, GEOMETRY = 3, FRAGMENT = 4, COMPUTE = 5};

	GLuint m_uIdProgram;		//id of the program
	GLuint m_vIdShader[6];	//ids of the loaded shaders; the 4th is empty always
	CGLSLProgram(void);
	~CGLSLProgram(void);
