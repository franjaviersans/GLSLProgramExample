...
CGLSLProgram::CGLSLProgram(void)
{
	...
	m_vIdShader[TESSELLATION_CONT] = 0;
	m_vIdShader[TESSELLATION_EVAL] = 0;
	m_vIdShader[GEOMETRY] = 0;
	m_vIdShader[FRAGMENT] = 0;
	m_vIdShader[COMPUTE] = 0;
...
void CGLSLProgram::loadShader(std::string strFileName, SHADERTYPE typeShader)
{
	...
	switch (typeShader)
  {
	...
	case TESSELLATION_CONT : { hShader = glCreateShader(GL_TESS_CONTROL_SHADER); break; }
    case TESSELLATION_EVAL : { hShader = glCreateShader(GL_TESS_EVALUATION_SHADER); break; }
...
void CGLSLProgram::create()
{
	...
	if(m_vIdShader[TESSELLATION_CONT] > 0)
		glAttachShader(m_uIdProgram, m_vIdShader[TESSELLATION_CONT]);
	if(m_vIdShader[TESSELLATION_EVAL] > 0)
		glAttachShader(m_uIdProgram, m_vIdShader[TESSELLATION_EVAL]);
	...
	glDeleteShader(m_vIdShader[TESSELLATION_CONT]);
	glDeleteShader(m_vIdShader[TESSELLATION_EVAL]);
